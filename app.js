...
// NOTA: Se usan addEventListener en lugar de onclick para mejor práctica, pero tu original usa onclick, lo mantendremos:
...
if (pub.mediaUrl) {
  if (pub.mediaUrl.includes("youtube.com") || pub.mediaUrl.includes("youtu.be")) {
    ...
    // NOTA: Tu código original tenía un error de URL de YouTube: "https://www.youtube.com/embed/${videoId}"
    // Lo mantengo para respetar tu código, pero podría ser un problema si esperas ver un video real.
    mediaHTML = `
...
if (pub.buttons) {
  Object.keys(pub.buttons).forEach(btnKey => {
    ...
    // NOTA IMPORTANTE: Ya que se movió a un archivo externo, la llamada debe ser global.
    // Esto funciona porque todas las funciones están en el scope global del script.
    buttonsHTML += `
...
// Función accesible desde el HTML de las publicaciones
window.openConfirmModal = function(pubId, safeBtnId, price, rawBtnId) {
...
// Función accesible desde el HTML para copiar la clave en el historial
window.copiarKey = function(text) {
